...
export function getRandomBool(weight) {
    if (!weight) weight = 2;
    return Math.round(getRandomFloat(1, weight)) == 1;
};
/**
 ...
 * @param {String} string the desired `Vector3`
 * @returns {SERVER.Vector3} the vector in string format
 */
export function stringToVec3(string) {
    const array = string.split(' ')
    return {x: Number(array[0]), y: Number(array[1]), z: Number(array[2])}
}
...
/**
 * Rounds a number to the nearest place
 * @param {Number} number 
 * @param {Number} place 
 * @returns 
 */
export function roundTo(number, place) {
...
export function getRealTime() {
  ...
  // Format the time with leading zeros if the value is less than 10
...
export function getMultipleRandomElements(arr, count) {
  ...
  // Slice the last 'count' elements (which are now randomly selected)
  return shuffled.slice(arr.length - count);
}
/**
 * Will run a command from the perspective of the world owner after a certan amount of ticks
 * @param {String} command The command to run
 * @param {Int} delay Delay in ticks
 */
export function runCommandDelay(command, delay) {
    SERVER.system.runTimeout(() => { SERVER.world.getAllPlayers()[0].runCommand(command) }, delay)
}
/**
 * Limites the range of numbers
 * @param {Number} num Existing Value
